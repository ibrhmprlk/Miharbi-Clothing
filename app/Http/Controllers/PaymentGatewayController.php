...
class PaymentGatewayController extends Controller
{
    ...
    private function payTR(Order $order)
    {
        try {
            ...
            // ✅ SSL doğrulama AÇık — production güvenliği
            ...
            if ($curl_error) {
                throw new \Exception('PayTR cURL hatası: ' . $curl_error);
            }
    ...
    // ─────────────────────────────────────────────
    // PayTR Webhook — hash doğrulamalı
    // ─────────────────────────────────────────────
    public function callback(Request $request)
    {
        try {
            ...
            $hash = base64_encode(hash_hmac(
                'sha256',
                $request->merchant_oid . $request->status . $request->total_amount . $merchant_salt,
                $merchant_key,
                true
            ));
            ...
            if ($request->status === 'success') {
                ...
                // ✅ Stok burada DÜŞÜRÜLMESİN — createOrderItems zaten düşürdü.
                // Sadece payment kaydı güncelle.
